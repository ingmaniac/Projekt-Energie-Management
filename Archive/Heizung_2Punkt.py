...
class NoCoordinatesToolbar(NavigationToolbar2Tk):
    def set_message(self, s):
        pass  # Deaktiviert Koordinatenanzeige

    def zoom(self, *args):
        super().zoom(*args)
        for ax in self.canvas.figure.axes:
            ax._zoom_mode = 'x'  # Nur X-Zoom aktiv
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.state('zoomed')  # Fenster beim Start maximieren
    ...
    def init_steuerung_panel(self):
        ...
        for name, label, default in parameters:
            lbl = customtkinter.CTkLabel(self.left_frame, text=label)
    ...
    def update_plot(self):
        ...
        try:
            T_umgebung_verlauf, zeitstempel = self.extract_temp_from_csv(self.selected_csv_path)
            n = len(T_umgebung_verlauf)

            if self.selected_tsoll_path:
                T_soll_verlauf, tsoll_zeitstempel = self.extract_temp_from_csv(self.selected_tsoll_path)
                if tsoll_zeitstempel != zeitstempel:
        ...
        T = 22  # Anfangstemperatur
        P_el_list, cop_list, temperaturen, energie_kum = [], [], [], []
        stromverbrauch_kWh = 0

        # Zweipunktregler-Hysteresegrenzen
        delta_on = -0.5   # Heizung an, wenn T < T_soll - 0.5
        delta_off = 0.5   # Heizung aus, wenn T > T_soll + 0.5
        heizung_an = False
        ...
        for i in range(n):
            ...
            # Zweipunktregler-Logik
            if not heizung_an and delta_T > abs(delta_on):
                heizung_an = True
            elif heizung_an and delta_T < -abs(delta_off):
                heizung_an = False

            # Heizleistung nur bei aktiver Heizung
            if heizung_an:
                P_th = 8000  # feste Heizleistung in Watt (z. B. 8 kW)
            else:
                P_th = 0

            cop = max(a * T_umg + b, cop_constant)
            P_el = min(P_th / cop if cop > 0 else 0, 10_000)
            P_v_dyn = P_el * cop
        ...
        for i, zeit in enumerate(x_axis[:-1]):
            if zeit.time() < time(6, 0) or zeit.time() >= time(22, 0):
                self.ax.axvspan(x_axis[i], x_axis[i + 1], color="lightblue", alpha=0.1)
    ...
    def generate_tsoll_csv(self):
        ...
        if self.selected_csv_path:
            temps, zeiten = self.extract_temp_from_csv(self.selected_csv_path)
            times = zeiten
        else:
        ...
        values = [21.0 if 6 <= t.hour < 22 else 17.0 for t in times]
        df = pd.DataFrame({
            "Zeit": [t.strftime("%d.%m.%Y %H:%M") for t in times],
            "Temperatur": values
        })
