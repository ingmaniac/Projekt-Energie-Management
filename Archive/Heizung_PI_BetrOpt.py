...
class App(customtkinter.CTk):
    ...
    def init_steuerung_panel(self):
        # Auskommentiert für Scrolloption
        ...
        params = [
            ("alpha", "Wärmeübergangskoeffizient alpha [W/m²·K]", 0.22),
    ...
    def update_plot(self):
        t_base = 600  # Zeitschritt: 10 Minuten in Sekunden
        ...
        try:
            opt = self.calculate_magnitude_optimum(
                alpha=params['alpha'],
                o=params['o'],
                c=params['c'],
                m=params['m'],
            ...
            k_p = opt["K_p"]
            k_i = opt["K_i"]
            print(f"Betragsoptimum: K_p = {k_p:.5f}, K_i = {k_i:.5e} (T_n = {opt['T_n']/3600/24:.2f} Tage)")
        except Exception as e:
        ...
        for i in range(n):
            ...
            P_th = (k_p * delta_T + k_i * integral_error)
            P_th = max(min(P_th, 20000), 0) #Begrenzung der Heizleistung 20kW
            cop = max(a * T_umg + b, cop_constant)
            if cop <= 0 or not isinstance(cop, (int, float)):
                cop = 1  # Fallback bei Fehler
            P_el = P_th / cop
            P_el = max(min(P_el, 10000), 0)
            P_v_dyn = P_th
            ...
            P_el_list.append(P_el / 1000) #P_el in kW
            cop_list.append(cop)
        ...
        ax2.set_ylabel("P_el (kW) , COP (1)", labelpad=15)
